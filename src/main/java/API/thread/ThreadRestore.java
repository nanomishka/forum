 ...
 */
public class ThreadRestore extends HttpServlet {
    private Connection myConn;
    public ThreadRestore(Connection myConnect) { myConn = myConnect; }

    public void doPost(HttpServletRequest request,
                       HttpServletResponse response) throws ServletException, IOException {
        System.out.println("Query: Thread restore");
        ...
        try{
            ...
            myStmt = myConn.prepareStatement("UPDATE threads SET isDeleted = false WHERE id = ?");
            myStmt.setString(1, jsonData.getString("thread"));
            myStmt.executeUpdate();
            myStmt.close();

            myStmt = myConn.prepareStatement("UPDATE posts SET isDeleted = false WHERE thread = ?");
